...
"""
Adapted script for clinical BHC fine-tuning, for Abstractive encoder/decoder models such as T5 and BART as well as 
Encoder/Decoder models such BERT-2-BERT. 
"""
...
class DataTrainingArguments:
    ...
    guidance_summ_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the summary guidance signal. Used during training for teacher forcing"},
    ),
    guidance_text_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the full texts guidance signal, used for eval and testing"},
    ),
...
def main(path=None):
    ...
    # expect a .json cfg file.
    if path is None:
        path = os.path.abspath(sys.argv[1])
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, Seq2SeqTrainingArguments))
    model_args, data_args, training_args = parser.parse_json_file(json_file=path)
    ...
    ds['train'].sort(src_col, reverse=True)
    val_test_ds = ds['test'].train_test_split(train_size=0.5, test_size=0.5, shuffle=False)
    val_ds = val_test_ds['train'].sort(src_col, reverse=True)
    test_ds = val_test_ds['test'].sort(src_col, reverse=True)
    raw_datasets = {
        'train': ds['train'],
        'validation': val_ds,
        'test': test_ds,
    ...
    # use the custom BART with guidance model
    model = BartWithGuidanceForConditionalGeneration.from_pretrained(model_args.model_name_or_path,
    ...
    guidance_text_column = data_args.guidance_text_column
    guidance_summ_column = data_args.guidance_summ_column
    ...
    def preprocess_train_function(examples):
        """ passes extracted guidance from target texts """
        return preprocess_function(examples, training=True)

    def preprocess_eval_test_function(examples):
        """ passes extracted guidance from the training texts """
        return preprocess_function(examples, training=False)

    def preprocess_function(examples, training=True):
        inputs = examples[text_column]
        targets = examples[summary_column]

        # guidance inferred from the training only
        guidance_inputs = examples[guidance_text_column]
        # guidance inferred from the the target summary.
        guidance_targets = examples[guidance_summ_column]
        ...
        # pre-process-train
        if training:
            guidance_decoder_labels = tokenizer(guidance_targets, max_length=max_target_length, padding=padding,
                                                truncation=True)
            model_inputs["input_guidance_ids"] = guidance_decoder_labels['input_ids']
            model_inputs["attention_guidance_mask"] = guidance_decoder_labels['attention_mask']
        else:  # vs pre-process-val / pre-process-test
            guidance_encoder_inputs = tokenizer(guidance_inputs, max_length=data_args.max_source_length, padding=padding,
                                                truncation=True)
            model_inputs["input_guidance_ids"] = guidance_encoder_inputs['input_ids']
            model_inputs["attention_guidance_mask"] = guidance_encoder_inputs['attention_mask']
        return model_inputs
    ...
    data_collator = DataCollatorForBartGuidanceSeq2Seq(
        tokenizer,
        model=model,
